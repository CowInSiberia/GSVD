...
def getsccodecore(eachLine):
    ...
    ContractFilePath = "D:\\SmartContracts\\BSC\\ContractsSourceCode"

    filename = eachLine[27:69]
...
def getsccode():
    ToCountPath = 'D:\\SmartContracts\\BSC\\'
    ...
    print(ToCount)
    try:
        SCAddress = open("D:\\SmartContracts\\BSC\\address.txt", "r")
        count = 0
...
def getSCAddress(eachurl, BasicInfoFilePath):
    ...
    for targetTR in targetTBody:
        if targetTR.name == 'tr':
            ...
            f0.write("https://bscscan.com" + targetTR.td.find('a', 'hash-tag text-truncate').attrs['href'] + "\n")
...
def updatescurl():
    ...
    urlList = ["https://bscscan.com/contractsVerified/1?filter=solc&ps=100",
               "https://bscscan.com/contractsVerified/2?filter=solc&ps=100",
               "https://bscscan.com/contractsVerified/3?filter=solc&ps=100",
               "https://bscscan.com/contractsVerified/4?filter=solc&ps=100",
               "https://bscscan.com/contractsVerified/5?filter=solc&ps=100"]

    # urlList = ["https://bscscan.com/contractsVerified/2?filter=solc&ps=100"]

    # urlList = ["https://polygonscan.com/contractsVerified?filter=solc"]

    # filepath是保存要爬取的智能合约地址的文件的存放路径
    # 请根据自己的需求改成自己想要的路径。
    BasicInfoFilePath = 'D:\\SmartContracts\\BSC\\'
